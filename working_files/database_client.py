...
def login(request):
    ...
    with grpc.insecure_channel(os.getenv("LOCALHOST") + ':1') as channel:
        stub = database_call_pb2_grpc.DatabaseCallStub(channel)
        response = stub.Login(database_call_pb2.LoginRequest(username=request['username'], password=request['password'], body=str(request)))
    return response
